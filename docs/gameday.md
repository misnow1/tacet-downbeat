...
| `Log not saving: ...` (red, under the link banner) | The last annotation or fader entry did not reach the disk: usually a full disk, sometimes a drive that went away. Fader moves are unaffected; the entries are not | Free space on the log's disk (`df -h`). The next tap saves again and the banner clears on its own. Write down what happened meanwhile on paper, since nothing can recover those annotations. If the message says the file has gone away, the box will not recreate it: restart the box with a fresh `--log` |
