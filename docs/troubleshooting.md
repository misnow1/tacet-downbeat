...
| The fader reads `unknown` instead of a number (#12) | You told the box **StageMix has it**. Every ramp starts from what the box believes the level is, and that belief is fiction the moment another interface can also move the fader - see game 2, where it said a confident `0.00 dB` for 76 minutes while StageMix actually had the DCA down | Ride the band from StageMix. Tap any fader-column button when you take it back |
| **Take back control. Where is the DCA now?** in the fader column | The first fader-column tap after **StageMix has it** is waiting on this answer before it does anything, so the ramp starts from the real level rather than the stale belief | Answer **It's up** or **It's down**, honestly, then the tap runs. A snap open (**Up on whistle** / **Up on drums**) skips this outright - it is correct from anywhere |
