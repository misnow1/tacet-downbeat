...
| The prompt stays open after you tap **Stand down** / **Arm** | Either of two things refused the answer, and the refusal shows on the strip while it is the same question, still on screen - not replaced by a new one. **Arm**, at an unknown fader position: refused for the same reason the bare **Arm** button would be (#107). **Stand down**, tapped late: it is a fader tap like any other and can be refused as stale, same as the row above | For an unknown-level Arm: tap **Close now**, or an open, then answer the same question again. For a stale Stand down: look at the band and the fader, and tap it again if you still mean it |
...
| `Not a target level: ...` (in the refusal line) | You changed the target to a level that is not one of the presets (`fader.presets`), so nothing was changed and nothing was logged. The segments on the page only offer presets, so this means the page is older than the box - the box restarted with a different list - or something other than the page sent the request | Reload the page and choose from the segments it shows now |
| `target -3 dB` in amber in the strip, or `target 0 dB (next open)` | The standing target is not the configured default, which is the first entry of `fader.presets`. Amber is there so a quiet setting left over from earlier is not forgotten. `(next open)` means a move in flight is heading somewhere other than the target: a ride that began before you changed it, or **Ready**'s own ride to its hold level (which is below the target, so this appears on every Ready ride, changed or not). The target applies to the next open, and nothing moves when it changes | If the amber level is what you want, nothing. Otherwise MORE > **Target level**, and tap the first segment |
| `target-set` in the log | You tapped a segment in MORE > **Target level** and it took: the entry has the level, the one it replaced and whether it is the default. Nothing moved when it was written - it only says which level the next open goes to. A change the box refused writes no entry | Nothing. Read it beside the `commanded` entries to see which level an open was meant for |
